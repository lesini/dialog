    ...
    Dialog.prototype = {
        ...
        create: function() {
            ...
            if (this.params.titleText) {
                contentHtml += '<h3 class="title">' + this.params.titleText + '</h3>';
            }

            if (this.params.close) {
                contentHtml += '<a class="dialog_close" href="javascript:;">+</a>';
            }

            if (this.params.content) {
                contentHtml += '<div class="dialog_content">' + this.params.content + '</div>';
            }

            if (this.params.okText || this.params.cancelTex) {
                var btnHtml = '';
                if (this.params.okText) {
                    btnHtml += '<a href="javascript:;" id="dialog_ok">' + this.params.okText + '</a>';
                }
                if (this.params.cancelText) {
                    btnHtml += '<a href="javascript:;" id="dialog_cancel">' + this.params.cancelText + '</a>';
                }
